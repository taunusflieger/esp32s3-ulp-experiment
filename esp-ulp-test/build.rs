...
fn main() -> anyhow::Result<()> {
    ...
    if cfg.get("esp32s3").is_some() {
        ...
        let mut ulp_elf = PathBuf::from("../esp32-ulp-blink/target/riscv32imc-unknown-none-elf");

        let profile = std::env::var("PROFILE")?;
        match profile.as_str() {
            "debug" => ulp_elf.push("debug"),
            "release" => ulp_elf.push("release"),
            _ => (),
        }

        ulp_elf.push("rust-esp32-ulp-blink");
